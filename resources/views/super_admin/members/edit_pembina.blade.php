...
@section('breadcrumbs')
    ...
    <span class="text-slate-700">Edit Pembina</span>
@endsection
    ...
    <div class="mb-8">
        <div class="flex items-start justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1.5 font-medium">Perbarui data dewan pembina.</p>
            </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="name" id="name" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors" placeholder="Contoh: Dr. Budi Santoso, M.Kom" value="{{ old('name', $member->user->name) }}">
                            @error('name')
                        ...
                        {{-- NIP --}}
                        <div class="sm:col-span-2">
                            <label for="nip" class="block text-sm font-bold text-slate-700 mb-1.5">NIP (Nomor Induk Pegawai) <span class="text-red-500">*</span></label>
                            <input type="text" name="nip" id="nip" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors" placeholder="Contoh: 198001012005011001" value="{{ old('nip', $member->user->nim) }}">
                            @error('nip')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="mt-6 flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-4">
