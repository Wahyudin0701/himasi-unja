...
@section('breadcrumbs')
    ...
    <span class="text-slate-700">Tambah Pembina</span>
@endsection
...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-slate-500 mt-2 font-medium">Input data pembina baru. Akun sistem akan dibuat otomatis.</p>
    </div>

    <form action="{{ route('super_admin.members.store.pembina') }}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="name" id="name" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors" placeholder="Contoh: Dr. Budi Santoso, M.Kom" value="{{ old('name') }}">
                            @error('name')
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="nip" id="nip" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors" placeholder="Contoh: 198001012005011001" value="{{ old('nip') }}">
                            @error('nip')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
