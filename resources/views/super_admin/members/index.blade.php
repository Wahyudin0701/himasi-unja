...
@section('breadcrumbs')
    ...
    <a href="{{ route('super_admin.dashboard') }}" class="text-slate-500 hover:text-brand-600 transition-colors">Sekretaris</a>
@endsection
...
<div class="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
    <div>
        <h1 class="text-2xl font-black text-slate-900 tracking-tight leading-tight">
            Data Pengurus HIMASI
        </h1>
        <p class="text-sm text-slate-500 mt-1.5 font-medium">Periode Aktif: {{ $activePeriod->name }}</p>
    </div>
</div>
...
<div id="section-pembina" class="mb-8 scroll-mt-24">
        <div class="flex items-center justify-between gap-2 mt-8 mb-4">
            ...
            <a href="{{ route('super_admin.members.create.pembina') }}" class="inline-flex items-center gap-2 px-3 py-2 sm:px-4 bg-white border border-slate-200 hover:border-brand-300 hover:text-brand-600 text-slate-700 text-sm font-bold rounded-lg transition-all shadow-sm">
                <i class="ph-bold ph-plus"></i>
                <span class="hidden sm:inline">Tambah Pembina</span>
            </a>
        </div>
    

    @if($pembinaMembers->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($pembinaMembers as $member)
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                            ...
                            <div class="flex items-center gap-2 shrink-0" @click.stop>
                                <a href="{{ route('super_admin.members.edit', $member->id) }}" class="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-white border border-slate-200 hover:border-emerald-300 hover:text-emerald-600 text-slate-600 text-[10px] sm:text-xs font-bold rounded-lg transition-colors">
                                    <i class="ph-bold ph-pencil-simple"></i>
                                    <span class="hidden sm:inline">Edit</span>
                                </a>
                                <form id="delete-form-{{ $member->id }}" action="{{ route('super_admin.members.destroy', $member->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDeleteMember({{ $member->id }})" class="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-red-50 border border-red-100 hover:bg-red-100 hover:border-red-200 text-red-600 text-[10px] sm:text-xs font-bold rounded-lg transition-colors">
...
<div id="section-dp" class="mb-8 scroll-mt-24">
    <div class="flex items-center justify-between gap-2 mt-8 mb-4">
        ...
        <a href="{{ route('super_admin.members.create.dp') }}" class="inline-flex items-center gap-2 px-3 py-2 sm:px-4 bg-white border border-slate-200 hover:border-brand-300 hover:text-brand-600 text-slate-700 text-sm font-bold rounded-lg transition-all shadow-sm">
            <i class="ph-bold ph-plus"></i>
            <span class="hidden sm:inline">Tambah Anggota DP</span>
        </a>
    </div>
    
    @if($dpMembers->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($dpMembers as $member)
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative group hover:border-amber-300 hover:shadow-md transition-all cursor-pointer"
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                            ...
                            <div class="flex items-center gap-2 shrink-0" @click.stop>
                                <a href="{{ route('super_admin.members.edit', $member->id) }}" class="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-white border border-slate-200 hover:border-amber-300 hover:text-amber-600 text-slate-600 text-[10px] sm:text-xs font-bold rounded-lg transition-colors">
                                    <i class="ph-bold ph-pencil-simple"></i>
                                    <span class="hidden sm:inline">Edit</span>
                                </a>
                                <form id="delete-form-{{ $member->id }}" action="{{ route('super_admin.members.destroy', $member->id) }}" method="POST" class="inline">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative group hover:border-brand-300 hover:shadow-md transition-all cursor-pointer"
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between gap-2">
                            ...
                            <a @click.stop href="{{ route('super_admin.members.edit', $member->id) }}" class="inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 bg-white border border-slate-200 hover:border-brand-300 hover:text-brand-600 text-slate-600 text-[10px] sm:text-xs font-bold rounded-lg transition-colors shrink-0">
                                <i class="ph-bold ph-pencil-simple"></i>
                                <span class="hidden sm:inline">Edit</span>
                            </a>
                        </div>
...
{{-- ===== DATA DIVISI LIST ===== --}}
<div>
    @foreach($divisiGrouped as $div)
        <div id="divisi-{{ $div->id }}" class="scroll-mt-24"></div>
    @endforeach
</div>
<div id="section-divisi" class="flex flex-col lg:flex-row gap-5 lg:gap-6 mb-8 items-start scroll-mt-24 relative" x-data="{ activeTab: window.location.hash.startsWith('#divisi-') ? parseInt(window.location.hash.replace('#divisi-', '')) : {{ $divisiGrouped->first()->id ?? 'null' }} }" x-init="window.addEventListener('hashchange', () => { if(window.location.hash.startsWith('#divisi-')) activeTab = parseInt(window.location.hash.replace('#divisi-', '')) })">
    @if($divisiGrouped->count() > 0)
        ...
        <div class="relative w-full min-w-0">
            @foreach($divisiGrouped as $div)
                            ...
                            @foreach($leaders as $leader)
                                <div class="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between gap-4 shadow-sm hover:border-indigo-200 transition-colors cursor-pointer"
                                    ...
                                    <div class="flex items-center shrink-0" @click.stop>
                                        <a href="{{ route('super_admin.members.edit', $leader->id) }}" class="inline-flex items-center gap-1 px-2 sm:px-2.5 py-0.5 sm:py-1.5 bg-white border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-600 text-xs font-bold rounded-lg transition-colors">
                                            <i class="ph-bold ph-pencil-simple"></i>
                                            <span class="hidden sm:inline">Edit</span>
                                        </a>
                                    </div>
                ...
                {{-- Action Bar --}}
                <div class="p-4 sm:p-5 flex justify-end gap-3 bg-white">
                    <a href="{{ route('super_admin.sub_divisions.index', $div->id) }}" class="inline-flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-700 text-sm font-bold rounded-lg transition-all shadow-sm">
                        <i class="ph-bold ph-list-dashes"></i>
                        <span>Kelola Bidang</span>
                    </a>
                    <a href="{{ route('super_admin.members.create', ['division_id' => $div->id]) }}" class="inline-flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-700 text-sm font-bold rounded-lg transition-all shadow-sm">
                        <i class="ph-bold ph-plus"></i>
                ...
                <div class="overflow-x-auto border-t border-slate-100">
                    <table class="w-full min-w-[1000px] text-sm text-left">
                        <thead class="text-xs text-slate-500 bg-slate-50 border-b border-slate-200 uppercase font-bold">
                            <tr>
                                <th scope="col" class="px-6 py-3.5 w-[35%]">Nama Pengurus</th>
                                ...
                                <th scope="col" class="px-6 py-3.5 w-[15%] text-center">Aksi</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-slate-50/50 transition-colors cursor-pointer"
                                    ...
                                    <td class="px-6 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2" @click.stop>
                                            <a href="{{ route('super_admin.members.edit', $member->id) }}" class="inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1.5 bg-white border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 text-slate-600 text-xs font-bold rounded-lg transition-colors">
                                                <i class="ph-bold ph-pencil-simple"></i>
                                                <span class="hidden sm:inline">Edit</span>
                                            </a>
                                            <form id="delete-form-{{ $member->id }}" action="{{ route('super_admin.members.destroy', $member->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDeleteMember({{ $member->id }})" class="inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1.5 bg-red-50 border border-red-100 hover:bg-red-100 hover:border-red-200 text-red-600 text-xs font-bold rounded-lg transition-colors">
                                                    <i class="ph-bold ph-trash"></i>
                                                    <span class="hidden sm:inline">Hapus</span>
                                                </button>
                                            </form>
