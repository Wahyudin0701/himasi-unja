        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors">
                        <option value="internal" {{ old('type', 'internal') == 'internal' ? 'selected' : '' }}>Internal Divisi</option>
                        <option value="kolaborasi" {{ old('type') == 'kolaborasi' ? 'selected' : '' }}>Kolaborasi Lintas Divisi</option>
                        <option value="event" {{ old('type') == 'event' ? 'selected' : '' }}>Event Kepanitiaan</option>
                    </select>
                    @error('type')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="sub_division_id" id="sub_division_id" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors">
                        ...
                        @foreach($subDivisions as $sub)
                            <option value="{{ $sub->id }}" {{ old('sub_division_id') == $sub->id ? 'selected' : '' }}>{{ $sub->name }}</option>
                        @endforeach
                    </select>
            ...
            <div id="kolaborasi-fields" class="space-y-6 hidden bg-purple-50 p-5 rounded-2xl border border-purple-200">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mb-3">Pilih divisi mana saja yang akan berkolaborasi dalam proker ini.</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        <label class="flex items-center p-3 border border-slate-200 rounded-xl bg-white cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="checkbox" name="partner_divisions[]" value="{{ $div->id }}" class="division-checkbox w-4 h-4 text-purple-600 bg-slate-100 border-slate-300 rounded focus:ring-purple-500">
                ...
                <div id="collaborators-container" class="hidden mt-4">
                    ...
                    <p class="text-xs text-slate-500 mb-3">Pilih anggota dari divisi mitra yang akan terlibat langsung dalam proker ini.</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ startDate: '{{ old('start_date') }}' }">
                {{-- Tanggal Mulai --}}
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" :min="startDate" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors" value="{{ old('end_date') }}">
                    @error('end_date')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea name="target_audience" id="target_audience" rows="3" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-brand-500 focus:border-brand-500 block p-3 transition-colors placeholder-slate-400" placeholder="Contoh: Seluruh Mahasiswa Sistem Informasi...">{{ old('target_audience') }}</textarea>
        ...
        async function fetchDivisionMembers() {
            ...
            try {
                ...
                collaboratorsList.innerHTML = allMembers.map(member => `
                    <label class="flex items-center p-3 border border-slate-200 rounded-xl bg-white cursor-pointer hover:bg-slate-50 transition-colors">
                        <input type="checkbox" name="collaborators[]" value="${member.id}" class="w-4 h-4 text-purple-600 bg-slate-100 border-slate-300 rounded focus:ring-purple-500">
